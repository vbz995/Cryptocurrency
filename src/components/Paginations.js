    ...
    console.log("this.prop", props.data)
        if(props.data!==undefined)
        {
            for(let i=0; i<Math.ceil(100/datasPerPages); i++){
                ...
                pageNumbers.push( <Pagination.Item onClick={()=>{props.SetActivePagination(i+1); props.FetchCryptos(i*datasPerPages+1, datasPerPages, props.selectedCurrency)}} active={(i+1)===props.activePagination}>{parseInt(i+1)}</Pagination.Item>)
            }
